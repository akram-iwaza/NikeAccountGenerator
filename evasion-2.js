(function () {
    const patchPlugins = (ctxWindow) => {
        ...
        const generateFunctionMocks = (proto, itemMainProp, dataArray) => ({
                ...
                apply(target, ctx, args) {
                    if (!args.length) {
                        throw new ctxWindow.TypeError(
                            `Failed to execute 'item' on '${proto[ctxWindow.Symbol.toStringTag]
                    ...
                    const isInteger = args[0] && ctxWindow.Number.isInteger(ctxWindow.Number(args[0])); // Cast potential string to number first, then check for integer

                    return (isInteger ? dataArray[ctxWindow.Number(args[0])] : dataArray[0]) || null;
                },
            }),
            ...
            namedItem: new ctxWindow.Proxy(proto.namedItem, {
                apply(target, ctx, args) {
                    if (!args.length) {
                        throw new ctxWindow.TypeError(
                            `Failed to execute 'namedItem' on '${proto[ctxWindow.Symbol.toStringTag]
                            }': 1 argument required, but only 0 present.`
                        );
                    }
        ...
        const generateMagicArray = (dataArray, proto, itemProto, itemMainProp) => {
            ...
            const patchItem = (item, data) => {
                ...
                const blacklist = [...ctxWindow.Object.keys(data), "length", "enabledPlugin"];

                return new ctxWindow.Proxy(obj, {
                    ownKeys(target) {
                        return ctxWindow.Reflect.ownKeys(target).filter((k) => !blacklist.includes(k));
                    },
        ...
        const patchNavigator = (name, value) => {
            replaceProperty(ctxWindow.Object.getPrototypeOf(ctxWindow.navigator), name, {
                get() {
                    return value;
                },
            });
    ...
    const rtt = possibleRttValues[Math.floor(Math.random() * possibleRttValues.length)];
